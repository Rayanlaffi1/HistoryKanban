...
func (s *Serveur) Routeur() *gin.Engine {
	...
	api.GET("/taches/:id/soustaches", s.listerSousTaches)
	api.POST("/taches/:id/soustaches", s.creerSousTache)
	api.PUT("/soustaches/:id", s.modifierSousTache)
	api.DELETE("/soustaches/:id", s.supprimerSousTache)
